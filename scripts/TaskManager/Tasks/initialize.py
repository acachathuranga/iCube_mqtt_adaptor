...
from tele_joy.msg import TeleJoy
from sensor_msgs.msg import Joy
from dock_indoor.msg import DockIndoorPub
...
class Initialize():
    ...
    def __init__(self, comHandler):
        ...
        self.robot_teleop_topic = rospy.get_param(rospy.get_name() + "/robot_teleop_topic", "/mobilemanual")
        self.robot_initial_pose_topic = rospy.get_param(rospy.get_name() + "/robot_initial_pose_topic", "/initialpose")
        self.top_camera_name = rospy.get_param(rospy.get_name() + "/top_camera_name", "/dev/TopCamera")
        self.top_camera_exposure = rospy.get_param(rospy.get_name() + "/top_camera_exposure", 150)
        
        self.audio_path = os.path.join(rospkg.RosPack().get_path("robot_mqtt_adaptor"), 'media')

        self.robot_dock_subscriber = rospy.Subscriber(self.robot_dock_topic, DockIndoorPub, self.dock_callback)
        self.robot_teleop_publisher = rospy.Publisher(self.robot_teleop_topic, TeleJoy, queue_size=200)
    ...
    def start_docking(self):
        ...
        self.robot_teleop_publisher.publish(command)

    def initialize_robot(self):
        # Set Camera Exposure
        subprocess.Popen(['v4l2-ctl','-d',self.top_camera_name,'-c','exposure_absolute=' + str(self.top_camera_exposure)])
